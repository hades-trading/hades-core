...
class OkxExchangeClient(Exchange):
    ...
    def place_buy_order(self, symbol: str, size: float, price: float):
        return self.tradeApi.place_order(instId=symbol, tdMode='isolated', side='buy', sz=str(size), px=str(price),
                                         ordType='post_only')

    def place_sell_order(self, symbol: str, size: float, price: float):
        return self.tradeApi.place_order(instId=symbol, tdMode='isolated', side='sell', sz=str(size), px=str(price),
                                         ordType='post_only')
...
class OrderSubscriber(OkxConnectionManager):
    def __init__(self, strategy: Strategy) -> None:
        ...
        super().__init__(conf.okx['ws_private'],
                         [Subscription(self.channel, {'instType': self.strategy.instrumentType})], conf, True)
...
class PositionSubscriber(OkxConnectionManager):
    def __init__(self, strategy: Strategy) -> None:
        ...
        super().__init__(conf.okx['ws_private'],
                         [Subscription(self.channel, {'instType': self.strategy.instrumentType}, interval=3)], conf,
                         True)
